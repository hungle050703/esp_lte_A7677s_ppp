...
// 
//     // Đảm bảo trạng thái nghỉ ban đầu (Tắt transistor)
//     gpio_set_level(MODEM_PWRKEY_PIN, 0); 
//     gpio_set_level(MODEM_RST_PIN, 0);    
//     vTaskDelay(pdMS_TO_TICKS(VBAT_STABLE_MS)); // Cho dien ap VBAT on dinh
//     
...
//     vTaskDelay(pdMS_TO_TICKS(PWR_ON_TIME_MS));
...
void app_main(void)
{
    ...
    // Bo qua vc kich PWRKEY (Nghi ngo mach tu dong bat nguon khi cam dien)
    // power_on_modem();
    ...
    // 5. Kiểm tra vòng lặp SIM sẵn sàng (Nhận sớm thoát sớm, tối đa chờ 60 giây)
    bool pin_ok = false;
    ESP_LOGI(TAG, "Dang tim the SIM (60s)...");
    for (int i = 1; i <= 60; i++) {
        if (esp_modem_read_pin(dce, &pin_ok) == ESP_OK && pin_ok) {
            ESP_LOGI(TAG, "=> SIM Card OK! (Mat %d giay de xac nhan)", i);
            break;
        }
    ...
    if (!pin_ok) {
        ESP_LOGE(TAG, "Khong tim thay SIM!");
    }
